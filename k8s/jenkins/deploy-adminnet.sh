...
docker run --rm \
  ...
  bash -lc "dotnet restore Admin.NET.sln && dotnet publish Admin.NET.Web.Entry/Admin.NET.Web.Entry.csproj -f net10.0 -c Release -r linux-x64 --self-contained true -o /src/publish/api /p:PublishSingleFile=false"
